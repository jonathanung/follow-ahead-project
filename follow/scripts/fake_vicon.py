...
class FakeVicon(Node):
    ...
    def publish_human(self):
        ...
        if mode == "stationary":
            x = self.get_parameter("stationary_x").get_parameter_value().double_value
            y = self.get_parameter("stationary_y").get_parameter_value().double_value
            ...
            # Base position after n complete zigzag cycles (net y is always 0 per cycle)
