...
# follow_ros1/scripts — contains planner.py, node.py, simple_grid.py, etc.
...
def _action_to_twist(action: str, robot_yaw: float) -> Twist:
    ...
    if action is None or action == 'STAY':
        return t  # zero velocity
 
    target_yaw = _ACTION_TO_YAW.get(action)
    if target_yaw is None:
        return t
    
    # Shortest angular error (wrapped to [-pi, pi])
    err = math.atan2(math.sin(target_yaw - robot_yaw),
                     math.cos(target_yaw - robot_yaw))

    ALIGN_THRESHOLD = 0.3   # radians (~17°) — below this, also drive forward
    ...
    if abs(err) > ALIGN_THRESHOLD:
        ...
        t.angular.z = TURN_SPEED * err   # proportional correction
